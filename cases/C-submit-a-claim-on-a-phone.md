...
**Group:** `groep-c` · **Engine:** `/prototype` (UI) · **Also:** `/research`, `/grill-with-docs` · **Starts on:** one prototype issue, one research issue, one grilling issue blocked by both
...
## Goal
Three UI variants side by side, a group choice made by looking at them, one research note, and one ADR that records the choice with its price.
...
2. The rest: in a second Claude Code session, `git switch -c prototype/submit-on-phone`, and run `/prototype` with the three variants in **one** `prototypes/submit-on-phone/index.html`: plain HTML, Dutch labels from `CONTEXT.md` (`_UI_`), no backend, fake data. First variant visible within a few minutes.
3. Open it on a real phone if you can (or the browser's device mode). Each person gets 1 minute per variant. Then vote, and write down why.
4. **Exchange (minute 20):** the prototype will show states you did not expect (a refused line? a missing receipt?). File one as a request for group D, whose state machine owns that. Take one issue from A's map if it fits.
5. Close the research issue with the note. `/grill-with-docs` on issue 3: the vote plus the research. ADR title with the tension, for example *"Photo first costs us a second pass for the amount"*.
...
7. PR, reviewer from another group. They open the prototype before they approve.
...
- Did the group choose, or did the prettiest variant win? What would a site worker with gloves choose?
